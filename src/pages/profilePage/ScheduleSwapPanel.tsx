...
type CourseSection = CourseScheduleFragment['sections'][number];
type CourseMeeting = CourseSection['meetings'][number];
...
const getOpenSeats = (section: CourseSection) =>
  Math.max(section.enrollment_capacity - section.enrollment_total, 0);

const formatDays = (days: string[]) =>
  weekDayLetters.filter((day) => days.includes(day)).join(' ');

const getProfessorHref = (professorName: string) => {
  const professorPath = professorName.trim().toLowerCase().replace(/\s+/g, '-');
  return `https://uwflow.com/professor/${encodeURIComponent(professorPath)}`;
};
...
const getMeetingTime = (meeting: CourseMeeting) => {
...
const MeetingInstructor = ({
  ...
  return (
    <div className="min-w-0">
      <a
        className="font-semibold text-professors underline-offset-2 hover:underline"
        href={getProfessorHref(professor.name)}
      >
        {professor.name}
      </a>
      {stats && (
...
const EmptyState = () => (
  <div className="flex min-h-[178px] flex-col items-center justify-center rounded border border-light3 bg-white px-6 py-8 text-center">
...
const ScheduleSectionRow = ({
  ...
  return (
    <div
      className={cn(
        'border-l-4 border-transparent bg-white px-5 py-4 outline-none transition-colors',
...
const ScheduleSwapPanel = ({
  ...
  const handleCourseChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    onPreviewChange(null);
    onCourseChange(value === '' ? null : Number(value));
  };
  ...
  return (
    <aside className="w-full rounded border border-light3 bg-white shadow-sm tablet:max-w-[360px]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-light3 px-4 py-3">
        <div className="inline-flex rounded border border-light3 bg-light1 p-1">
          {availableTerms.map((term) => (
            <button
              className={cn(
                'h-8 rounded px-3 text-sm font-semibold text-dark2 transition-colors',
        ...
        {onClose && (
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded border border-light3 text-dark2 hover:bg-light1 hover:text-dark1"
            onClick={handleClose}
            type="button"
          >
      ...
      <div className="flex items-center gap-2 border-b border-light3 px-4 py-3">
        ...
        <select
          className="h-9 min-w-0 flex-1 rounded border border-light3 bg-white px-3 text-sm font-semibold text-courses outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
      ...
      {!selectedCourse ? (
        ...
        <div>
          <div className="border-b border-light3 px-4 py-3">
            <div className="flex items-baseline gap-2">
              <h2 className="text-base font-semibold text-courses">
            ...
            <p className="mt-1 text-sm text-dark3">{selectedCourse.name}</p>
          </div>
          {sections.length === 0 ? (
            ...
            <div className="divide-y divide-light3">
